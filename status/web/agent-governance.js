...
function render(data) {
  ...
  byId("decision-copy").textContent = decision.state === "READY"
    ? "冻结候选、构建物与全部验收 Oracle 已形成可追溯证据。"
    : "系统按 fail-closed 原则展示真实缺口；缺失或过期数据不会沿用旧绿色。";

  const m = data.metrics || {};
  byId("metric-runs").textContent = safe(m.run_count ?? 0);
  byId("metric-gates").textContent = safe(m.gate_count ?? 0);
  byId("metric-pass").textContent = safe(m.pass_count ?? 0);
  byId("metric-candidates").textContent = safe(m.candidate_count ?? 0);
  ...
  </tr>`).join("") : emptyRow(6, "尚无经验候选；系统不会自动安装 Skill");
